...
const featuresData = [
      ...
      <div className="bg-[#f8fafc] rounded-2xl h-56 mb-8 p-6 flex flex-col justify-between border border-gray-100/50 relative overflow-hidden" aria-hidden="true">
        ...
        <div className="flex items-end justify-between gap-2 h-24 mt-4" role="img" aria-label="WPM progress chart showing 7 data points">
          {[40, 60, 45, 80, 55, 90, 75].map((height, i) => (
            <div
              key={i}
              className="w-full bg-[#126dfb] rounded-t-md hover:opacity-80 transition-opacity duration-300 cursor-default"
              style={{ height: `${height}%` }}
              aria-hidden="true"
            />
          ))}
    ...
    illustration: (
      <div className="bg-[#f8fafc] rounded-2xl h-56 mb-8 p-6 flex flex-col justify-center gap-4 border border-gray-100/50 relative overflow-hidden" aria-hidden="true">
        ...
        <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none drop-shadow-xl">
          <div className="bg-white p-3 rounded-full shadow-lg transform hover:scale-110 transition-transform duration-300 border border-gray-50">
            <Image
  ...
  },
  {
    ...
    illustration: (
      <div className="bg-[#f8fafc] rounded-2xl h-56 mb-8 relative border border-gray-100/50 flex items-center justify-center overflow-hidden" aria-hidden="true">
        ...
        {/* Center Node Logo */}
        <div className="w-14 h-14 bg-[#ffffff] rounded-2xl z-10 flex items-center justify-center shadow-lg shadow-blue-500/20 transform hover:rotate-12 transition-transform duration-500 border border-gray-50">
          <Image
        ...
        <div className="absolute top-8 left-10 bg-white px-4 py-2 rounded-full text-xs font-semibold shadow-sm border border-gray-100 text-gray-700 z-10 flex items-center gap-2" aria-hidden="true">
          <div className="w-2 h-2 rounded-full bg-orange-400 animate-pulse" /> 7 Day Streak
        </div>
  ...
  },
  {
    ...
    illustration: (
      <div className="bg-[#f8fafc] rounded-2xl h-56 mb-8 p-6 flex flex-col justify-between border border-gray-100/50 overflow-hidden relative group" aria-hidden="true">
        ...
        <div className="flex items-center justify-between mb-4 z-10">
          ...
          <div className="flex gap-2" aria-hidden="true">
            ...
            <span className="w-12 h-4 bg-gray-800 rounded-full group-hover:bg-[#126dfb] transition-colors" />
          </div>
        </div>
        
        {/* Custom SVG Smooth Line Chart */}
        <div className="absolute bottom-0 left-0 right-0 h-[140px] w-full" role="img" aria-label="Performance trend chart showing upward trajectory">
          <svg className="w-full h-full drop-shadow-sm group-hover:scale-[1.02] transition-transform duration-500 origin-bottom" viewBox="0 0 100 40" preserveAspectRatio="none">
            <defs>
              <linearGradient id="featureLineGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#126dfb" stopOpacity="0.2" />
                <stop offset="100%" stopColor="#ffffff" stopOpacity="0" />
              </linearGradient>
            </defs>
            {/* Area Fill */}
            <path d="M0,35 C20,30 35,15 50,20 C65,25 80,10 100,5 L100,40 L0,40 Z" fill="url(#featureLineGradient)" />
            {/* Smooth Line */}
            <path d="M0,35 C20,30 35,15 50,20 C65,25 80,10 100,5" fill="none" stroke="#126dfb" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            {/* Mid Data Point */}
            <circle cx="50" cy="20" r="1.5" fill="#ffffff" stroke="#126dfb" strokeWidth="1" />
            {/* Peak Data Point */}
            <circle cx="99.5" cy="5.5" r="2.5" fill="#ffffff" stroke="#126dfb" strokeWidth="1.5" className="group-hover:animate-pulse" />
          </svg>
        </div>
  ...
  return (
    <section className="py-24 bg-white overflow-hidden" aria-labelledby="features-heading">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div className="inline-flex items-center justify-center px-4 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-[#126dfb] text-sm font-medium mb-6 uppercase tracking-wider" role="status">
            Real-time typing test
          </div>
          <h2 id="features-heading" className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 tracking-tight leading-[1.1] mb-6">
        ...
        >
          {featuresData.map((feature) => (
            <motion.article
              ...
              key={feature.id}
              className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-xl hover:border-blue-100/50 transition-all duration-300 group cursor-default"
              role="listitem"
            >
              <div className="mb-8" aria-hidden="true">
                {feature.illustration}
              </div>
              <h3 className="text-[22px] font-bold text-gray-900 mb-3 tracking-tight">{feature.title}</h3>
